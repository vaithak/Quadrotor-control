...
def simulate(initial_state, quadrotor, controller, trajectory, t_final, terminate=None, vio = None, stereo = None):
    ...
    replan_num = math.ceil(trajectory.t_check_traj / t_step)
    num = 0
    while True:
        num += 1
        ...
        cur_time = time[-1] + t_step
        cur_state = quadrotor.step(state[-1], control[-1]['cmd_motor_speeds'], t_step)
        time.append(cur_time)
        state.append(cur_state)

        if num > replan_num:
            if trajectory.local:
                trajectory.replan(cur_state, cur_time)
            num = 0
...
def sanitize_control_dic(control_dic):
    ...
    """
    control_dic['cmd_motor_speeds'] = np.asarray(control_dic['cmd_motor_speeds'],
                                                 np.float64).ravel()
