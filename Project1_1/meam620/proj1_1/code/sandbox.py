...
initial_state = {'x': np.array([0, 0, 0]),
                 'v': np.zeros(3,),
                 'q': Rotation.from_euler('zyx', [0, 0, -10], degrees=True).as_quat(),
                 'w': np.zeros(3,)}
...
q = state['q']
ax = axes[0]
ax.plot(time, Rotation.from_quat(q_des).as_euler('zyx', degrees=True)[:,0], 'r', time, Rotation.from_quat(q_des).as_euler('zyx', degrees=True)[:,1], 'g', time, Rotation.from_quat(q_des).as_euler('zyx', degrees=True)[:,2], 'b')
ax.plot(time, Rotation.from_quat(q).as_euler('zyx', degrees=True)[:,0], 'r.',    time, Rotation.from_quat(q).as_euler('zyx', degrees=True)[:,1], 'g.',    time, Rotation.from_quat(q).as_euler('zyx', degrees=True)[:,2], 'b.')
