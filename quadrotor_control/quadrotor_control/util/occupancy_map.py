...
class OccupancyMap:
    def __init__(self, world=World.empty((0, 2, 0, 2, 0, 2)), resolution=(.1, .1, .1), margin=.2, local_range=(5.0, 5.0, 5.0)):
        """
        ...
        Parameters:
            ...
            local_range, local update range
        """
        self.world = world
    ...
    def update(self, pos):
        local_min = pos - self.local_range
        local_max = pos + self.local_range
        # Iterate through each block obstacle.
        for block in self.world.world.get('blocks', []):
            # Get cropped block
            extent = block['extents'].copy()
            right_point  = np.array([extent[1], extent[3], extent[5]])
            left_point   = np.array([extent[0], extent[2], extent[4]]) #min point
            if (left_point>local_max).any() or (right_point<local_min).any():
                continue
            for i in range(0, 3):
                extent[2*i]   = max(local_min[i], extent[2*i])
                extent[2*i+1] = min(local_max[i], extent[2*i+1])
