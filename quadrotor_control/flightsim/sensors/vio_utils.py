...
#######################################################################
from quadrotor_control.code.vio import *
################################################################################################################################################################
...
class Vio():
    ...
    def initialize(self, gt_state, gt_acceleration, sim_time):
        ...
        if self.initialized == False:
            ...
            # The covariance matrix should be initialized to zero since our nominal state is taken from simulator ground truth (i.e. the initial state is perfect)
            self.error_state_covariance = np.zeros((18,18))
            ...
            if valid_features_normalized_uv.shape[0] == 0:
                ...
                if self.prev_uv is None:
                    ...
                    if len(ind_common) == 0:
                        ...
                        # Keep the most recent frame's features and state to make feature tracking more robust
                        self.prev_uv = uv_new
                        self.prev_depth = depth
                        self.prev_index_of_valid_features = index_of_valid_features
                        self.last_R = self.nominal_state[2].as_matrix()
